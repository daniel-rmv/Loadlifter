#!/usr/bin/env python3
# buzzer_driver.py
# Low-level buzzer backend selection.
...
class BuzzerDriver:
    ...
    gpio_pin: BCM pin (e.g. 6) when backend="gpio"
    gpio_active_high: True means active HIGH, False means active LOW
    pwm_hz: 0 = no PWM (active buzzer), >0 = PWM frequency (passive buzzer, RPi.GPIO only)
    """
    def __init__(self, backend="auto", gpio_pin=None, gpio_active_high=True, pwm_hz=0):
        ...
        if not self.ok:
            print("[BuzzerDriver] No suitable backend found.")
    ...
    def _try_init_demo(self):
        ...
        self.ok = True
        print("[BuzzerDriver] Demo backend active (buzzer_control_demo.py)")
    ...
    def _try_init_gpiod(self):
        try:
            ...
            print("[BuzzerDriver] libgpiod not available:", e)
            return

        if self.gpio_pin is None:
            print("[BuzzerDriver] gpiod: GPIO pin not configured (--buzzer_pin).")
            return
